...
static void check_cat_shape_except_dim(
    const Tensor& first,
    const Tensor& second,
    int64_t dimension,
    size_t index) {
...
static c10::MemoryFormat cat_compute_output_memory_format(
...
TORCH_PRECOMPUTE_META_FUNC(cat)(const ITensorListRef& tensors, int64_t dim) {
  ...
  return TORCH_PRECOMPUTE_STRUCT(cat)()
      .set_dim(dim)
      .set_valid(static_cast<int64_t>(valid))
...
static void check_cat_sparse_dims(
    Tensor const& t,
    size_t pos /* used only for debug messages */,
    IntArrayRef sizes,
    int64_t wrapped,
    int64_t sparse_dim,
    int64_t dense_dim) {
...
template <typename T>
inline static void setStridedUnchecked(
    const Tensor& self,
    ArrayRef<T> size,
    ArrayRef<T> stride,
    T&& storage_offset) {
...
const Tensor& as_strided__symint(
  ...
  auto storage_offset =
      std::move(storage_offset_).value_or(self.sym_storage_offset());
...
Tensor select_symint(const Tensor& self, int64_t dim, c10::SymInt index) {
  ...
  if (self.is_quantized()) {
    ...
    result =
        as_strided_qtensorimpl(self, sizes, strides, storage_offset, quantizer);
  } else {
...
inline static Tensor view_impl(const Tensor& self, IntArrayRef size) {
...
Tensor movedim(const Tensor& self, IntArrayRef src, IntArrayRef dst) {
  ...
  int64_t rest_dim = self.dim() - static_cast<int64_t>(src.size());
  TORCH_INTERNAL_ASSERT(
