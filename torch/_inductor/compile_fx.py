    ...
    **graph_kwargs: Unpack[_CompileFxKwargs],
    ...
    if log.isEnabledFor(logging.INFO):
        ...
        for key, value in counters["aten_mm_info"].items():
            b, m, n, k = key.split("_")[-4:]
            name = "_".join(key.split("_")[:-4])
            mm_table_data.append([name, b, m, n, k, value])
        log.info("Overview info of inductor aten mms: ")
        log.info(
            "{:<20} | {:<20} | {:<20} | {:<20} | {:<20} | {:<20}".format(  # noqa: G001
                "Name", "B", "M", "N", "K", "Count"
            )
        )
        log.info("-" * 120)
        for row in mm_table_data:
            log.info("{:<20} | {:<20} | {:<20} | {:<20} | {:<20} | {:<20}".format(*row))  # noqa: G001
            log.info("-" * 120)
