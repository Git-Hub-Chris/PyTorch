...
def wrap_command_with_numa_bindings(
    *,
    ...
    if numa_options.affinity_mode == AffinityMode.NODE:
        numactl_options = _get_node_numactl_options(gpu_index=gpu_index)
    elif numa_options.affinity_mode == AffinityMode.SOCKET:
        numactl_options = _get_socket_numactl_options(gpu_index=gpu_index)
    elif numa_options.affinity_mode == AffinityMode.EXCLUSIVE:
        numactl_options = _get_exclusive_numactl_options(gpu_index=gpu_index)
    elif numa_options.affinity_mode == AffinityMode.CORE_COMPLEX:
        numactl_options = _get_core_complex_numactl_options(gpu_index=gpu_index)
    else:
    ...
    return (NUMACTL_COMMAND, *numactl_options, *command_args)


def _raise_if_numactl_not_available() -> None:
    if not shutil.which(NUMACTL_COMMAND):
        raise RuntimeError(
            f"{NUMACTL_COMMAND} shell command is required for NUMA bindings."
        )


def _get_node_numactl_options(*, gpu_index: int) -> Sequence[str]:
    """
...
def _get_socket_numactl_options(*, gpu_index: int) -> Sequence[str]:
    """
...
def _get_exclusive_numactl_options(*, gpu_index: int) -> Sequence[str]:
    """
...
def _get_core_complex_numactl_options(*, gpu_index: int) -> Sequence[str]:
    """
    ...
    # Sort by number of available cpus (dicts maintain order in Python)
    max_level_cache_to_allowed_logical_cpu_indices = dict(
        sorted(
            max_level_cache_to_allowed_logical_cpu_indices.items(),
            key=lambda cache_and_logical_cpu_indices: (
                # Sort by number of CPUs available
                len(cache_and_logical_cpu_indices[1]),
                # Then, sort by cache key for consistency.
                cache_and_logical_cpu_indices[0],
            ),
            reverse=True,
        )
    )
...
def _get_systemwide_numa_node_indices() -> set[int]:
    with open("/sys/devices/system/node/possible") as f:
        possible_nodes_str = f.read().strip()
